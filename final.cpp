...
void train_neural_net()
{
    while (epoch < 20000)
    {
        // For all the 4 elements of the training set
        for (int i = 0; i < 4; i++)
        {
...
void calc_output_neuron()
{
    sum_output = (h1 * weights[6]) + (h2 * weights[7]) + (bias * weights[8]);
    output_neuron = sigmoid_function(sum_output);
    ///std::cout << "Outs: " << sum_h1 << "," << sum_h2 << ", " << sum_output << std::endl;
    std::cout << std::endl;
}
...
void calc_derivatives(int x)
{
    ...
    ///std::cout << "Derivatives(Out,1,2): " << derivative_O1 << ", " << derivative_h1 << ", " << derivative_h2 << std::endl;
}

static inline double gradient(double input, double derivative) { return (sigmoid_function(input) * derivative); }
static inline double gradient(double derivative)               { return (sigmoid_function(bias) * derivative); }

void calc_gradient(int x) {
    gradients[0] = gradient(training_data[x][0], derivative_h1);
    gradients[2] = gradient(training_data[x][1], derivative_h1);

    gradients[1] = gradient(training_data[x][0], derivative_h2);
    gradients[3] = gradient(training_data[x][1], derivative_h2);

    gradients[6] = gradient(h1, derivative_O1);// h1 * derivative_O1;
    gradients[7] = gradient(h2, derivative_O1);// h2 * derivative_O1;

    // Gradients for the bias components
    gradients[4] = gradient(derivative_h1);
    gradients[5] = gradient(derivative_h2);
    gradients[8] = gradient(derivative_O1);
    /*std::cout << "Gradient[" << x << "]: ";
    for (int i = 0; i<9; i++)
...
void calc_updates()
{
    for (int i = 0; i < 9; i++) {
        ...
        ///std::cout << "deltaWeight := " << update_weights[i] << "=" << learning_rate <<"*"<< gradients[i] << "+"<< momentum*prev_weight_update[i] << std::endl;
    ...
    /*std::cout << "update_weights: ";
    for (int i = 0; i<9; i++)
        std::cout << update_weights[i] << ",";
    std::cout << std::endl;

    std::cout << "prev_weight_update: ";
    for (int i = 0; i<9; i++)
        std::cout << prev_weight_update[i] << ",";
    std::cout << std::endl;
...
void calc_RMSE_ERROR()
{
    RMSE_ERROR = sqrt((pow(error[0], 2) + pow(error[1], 2) + pow(error[2], 2) + pow(error[3], 2) ));
    ///std::cout << error[0] << " " << error[1] << " " << error[2] << " " << error[3] << std::endl;
